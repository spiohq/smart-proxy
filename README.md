...
| Malformed requests waste a rate-limit token | Proxy-side OpenAPI validation catches bad requests before they reach upstream |
...
| `X-SP-Proxy-Skip-Validation: true` | Bypass OpenAPI request validation for this request (trusted callers only) |
...
| `X-SP-Proxy-Validation: rejected` | Present on 400 responses generated by proxy-side OpenAPI validation |
...
| `SP_PROXY_VALIDATION_SPECS_URL` | GitHub ZIP of `amzn/selling-partner-api-models` | URL of the SP-API models ZIP archive to download |
| `SP_PROXY_VALIDATION_SPECS_DIR` | auto (next to binary) | Directory where downloaded spec files are cached |
| `SP_PROXY_VALIDATION_REFRESH_INTERVAL` | `24h` | How often to refresh the spec files from the source URL |
...
flowchart TD
    ...
    Val["OpenAPI Validation (optional)"]
    ...
    Client --> PM --> RL --> MR --> Val
    Val -- "invalid (400)" --> Client
    Val -- "valid / skip" --> RDT --> Cache
...
Every request passes through the middleware stack in order: Prometheus metrics collection, request/response logging, merchant identity resolution, proxy-side OpenAPI validation (when enabled), Auto-RDT token handling (if enabled), cache lookup, rate limiting, and finally the reverse proxy to Amazon. Responses flow back through the same stack, with PII redaction applied before logging and caching.

Validation runs after logging so rejected requests appear in the dashboard alongside real upstream calls, with a clear `X-SP-Proxy-Validation: rejected` marker in the response headers.
...
When `SP_PROXY_VALIDATION_ENABLED=true`, the proxy validates every incoming request against the official SP-API OpenAPI specs before forwarding to Amazon:

- **Catches bad requests before they cost a rate-limit token.** Malformed parameters, missing required fields, and wrong types return a precise `400` immediately, without touching upstream.
- **SP-API-native error format.** Validation rejections are returned as the standard `{"errors":[{"code":"InvalidInput","message":"..."}]}` envelope, identical to what Amazon itself would return, so existing error-handling code needs no changes.
- **Auto-updating specs.** The proxy downloads the full SP-API model ZIP from GitHub on startup and refreshes it on a configurable schedule (default: every 24 hours). No restarts required.
- **Graceful degradation.** Unknown or private-beta endpoints are passed through unchanged. The validator only rejects requests it has a spec for.
- **Per-request bypass.** Send `X-SP-Proxy-Skip-Validation: true` to bypass validation for a single request (for trusted internal callers only).
- **Zero body validation overhead.** Body validation is intentionally not performed. SP-API feeds and reports flow over S3, not the API itself, so body validation would add latency with no practical benefit.

For full configuration options and behavioral details, see [docs/VALIDATION.md](docs/VALIDATION.md).
...
| [Input Validation](docs/VALIDATION.md) | OpenAPI request validation, spec refresh, bypass options |
